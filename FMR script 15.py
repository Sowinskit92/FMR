...
if __name__ == "__main__":
    ...
    if kW_revenue == True:       
        ...
        def kW_revenue_by_service(BOD_data, DSP_data, EAC_data, STOR_data, SFFR_data):
            ...
            BM_capacities = BOD_data.groupby("NGU ID")["Cumulative capacity"].max().to_dict()
            # print(BM_capacities)
            ...
            units = sorted(list(set(list(STOR_capacities.keys()) + list(SFFR_capacities.keys())
                                    + EAC_capacities.index.values.tolist() + list(BM_capacities.keys())
                                    )))
            ...
            kW_revenue["Company"] = kW_revenue.index.map(NGU_company_dict2)
            kW_revenue["Company"] = kW_revenue.index.map(NGU_company_dict1).where(kW_revenue["Company"].isna(), kW_revenue["Company"])
            
            # once again, neither fuel type dicts get all assets by themselves
            kW_revenue["Fuel type"] = kW_revenue.index.map(NGUID_fuel_type_dict2)
            kW_revenue["Fuel type"] = kW_revenue.index.map(NGUID_fuel_type_dict1).where(kW_revenue["Fuel type"].isna(), kW_revenue["Fuel type"])
        ...
        kW_revenue, capacity_by_service, revenue_by_service = kW_revenue_by_service(BOD_data, DSP_data, EAC_data, STOR_data, SFFR_data)
        
        flex_fuel_types = ["Aggregated Unit", "Battery", "Diesel", "DSF: Distributed generation (for export)",
                           "DSF: Load response", "DSF: Storage (for export)", "DSF: Storage (onsite)", "Gas Recip", 
                           "OCGT"]
        
        # print(kW_revenue["Fuel type"].unique().tolist())
        
        company_kW_revenue = kW_revenue.groupby("Company")[kW_revenue.columns.tolist()[:-4]].sum()
        print(company_kW_revenue)
        
        
        raise TypeError("company_kW_revenue hasn't been tested yet")
        

        
        flex_kW_revenue = kW_revenue[kW_revenue["Fuel type"].isin(flex_fuel_types)]
        
        if Load == True:
            clear_range = f"A:{num_to_col(2*len(kW_revenue.columns.tolist()) + 4)}" # 2* for the two dataframes
            print(clear_range)
            Excel_load("Asset Revenue", kW_revenue, "A1", name = "£/kW revenue by unit by service", clear_range = clear_range)
            Excel_load("Asset Revenue", flex_kW_revenue, cell_ref = (0, len(kW_revenue.columns.tolist()) + 2), name = "£/kW revenue of flex units")
