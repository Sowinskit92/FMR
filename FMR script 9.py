...
def Data_load(data: str, date_from: str = False, date_to: str = False, BMUID_NGUID_dict = False, 
    ...
    class SQL_query: # creates SQL query class
        ...
        def EAC_data(date_from: str, date_to: str, connection = connection):
            ...
            WHERE DeliveryStartDate >= '{date_from}' AND DeliveryStartDate <= '{date_to}'"""
...
if __name__ == "__main__":
    ...
    if EAC == True:
        ...
        EAC_data = Data_load("EAC_data", date_from = "2023-11-01", date_to = date_to, NGUID_BMUID_dict = NGUID_BMUID_dict, 
                             NGUID_fuel_type_dict = NGUID_fuel_type_dict)
        ...
        # response_services = [i for i in EAC_data["Service"].unique().tolist() if "D" in i]
        # reserve_services = [i for i in EAC_data["Service"].unique().tolist() if ("P" in i) or ("N" in i)]
        
        
        #print(response_services)
